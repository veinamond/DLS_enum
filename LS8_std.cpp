				...
				for (CR[35] = AllN ^ (Rows[4] | Columns[3] | AD); CR[35] != 0; CR[35] = (CR[35] - 1) & CR[35]) {
					...
					for (CR[28] = AllN ^ (Rows[3] | Columns[4] | AD); CR[28] != 0; CR[28] = (CR[28] - 1) & CR[28]) {
						...
						for (CR[21] = AllN ^ (Rows[2] | Columns[5] | AD); CR[21] != 0; CR[21] = (CR[21] - 1) & CR[21]) {
							...
							Columns[5] |= LS[21];
							AD |= LS[21];
							CR[14] = AllN ^ (Rows[1] | Columns[6] | AD);
							if (CR[14] != 0) {
								LS[14] = (-CR[14]) & CR[14];
								Rows[1] |= LS[14];
								Columns[6] |= LS[14];
								for (CR[9] = AllN ^ (Rows[1] | Columns[1] | MD); CR[9] != 0; CR[9] = (CR[9] - 1) & CR[9]) {
									LS[9] = (-CR[9]) & CR[9];
									Rows[1] |= LS[9];
									Columns[1] |= LS[9];
									MD |= LS[9];
									for (CR[18] = AllN ^ (Rows[2] | Columns[2] | MD); CR[18] != 0; CR[18] = (CR[18] - 1) & CR[18]) {
										LS[18] = (-CR[18]) & CR[18];
										Rows[2] |= LS[18];
										Columns[2] |= LS[18];
										MD |= LS[18];
										for (CR[27] = AllN ^ (Rows[3] | Columns[3] | MD); CR[27] != 0; CR[27] = (CR[27] - 1) & CR[27]) {
											LS[27] = (-CR[27]) & CR[27];
											Rows[3] |= LS[27];
											Columns[3] |= LS[27];
											MD |= LS[27];
											for (CR[36] = AllN ^ (Rows[4] | Columns[4] | MD); CR[36] != 0; CR[36] = (CR[36] - 1) & CR[36]) {
												LS[36] = (-CR[36]) & CR[36];
												Rows[4] |= LS[36];
												Columns[4] |= LS[36];
												MD |= LS[36];
												for (CR[45] = AllN ^ (Rows[5] | Columns[5] | MD); CR[45] != 0; CR[45] = (CR[45] - 1) & CR[45]) {
													LS[45] = (-CR[45]) & CR[45];
													Rows[5] |= LS[45];
													Columns[5] |= LS[45];
													MD |= LS[45];
													for (CR[54] = AllN ^ (Rows[6] | Columns[6] | MD); CR[54] != 0; CR[54] = (CR[54] - 1) & CR[54]) {
														LS[54] = (-CR[54]) & CR[54];
														Rows[6] |= LS[54];
														Columns[6] |= LS[54];
														MD |= LS[54];
																																																							...
																																																							if (CR[38] != 0) {
																																																								...
																																																								if (CR[39] != 0) {
																																																									...
																																																									if (SquaresCnt % 10000000 == 0) {
																																																										...
																																																										cout << "Average performance: " << (double)(SquaresCnt) / (cpuTime() - start_time) << " squares per second" << endl;
																																																									}
																																																								}
																																																								Rows[4] ^= LS[38];
																																																							}
